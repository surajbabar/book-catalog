...
record.Help = record.fs.readFileSync("help.txt","utf-8");
record.Inventory = function () {
	var catalog =record.fs.readFileSync('book_inventry.txt','utf-8');
	return  catalog.split('\r\n');
};
record.IsDuplicate = function ( Newisbn ) {
...
record.modifyCatalog = function (input) {
     ...
     if(option.add) {
    	...
    	if(!NewBookRecord.price  || NewBookRecord.price ==" ") NewBookRecord.price = '-';
    	if(!NewBookRecord.author || NewBookRecord.author ==" ") NewBookRecord.author = '-';
    	if(!NewBookRecord.title  || NewBookRecord.title ==" ") NewBookRecord.title = '-';
    	if(!NewBookRecord.publisher || NewBookRecord.publisher ==" ") NewBookRecord.publisher = '-';
    	var Book = {};
    		Book.isbn=NewBookRecord.isbn;
    		Book.price=NewBookRecord.price;
    		Book.author=NewBookRecord.author;
    		Book.title=NewBookRecord.title;
    		Book.publisher=NewBookRecord.publisher;
    	 	Book.tags =[];
    	if(!Book.isbn || Book.isbn==" ") 
    		return "please mention  isbn with isbn number of book";
		if(record.IsDuplicate(Book.isbn)) 
			return "A book having isbn number:"+Book.isbn+" is already exists";
     	record.add(JSON.stringify(Book)+'\r\n');
     	    return "New book "+NewBook+" added successfully";
     } 
     if(option.remove) {
		var Newisbn = NewBook[1];
